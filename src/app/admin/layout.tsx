...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isHydrated) return <main className="min-h-screen bg-background" />;
      ...
      <main className="min-h-screen bg-background flex items-center justify-center p-6">
        <div className="w-full max-w-md space-y-8 graceful-reveal">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-4xl font-headline uppercase tracking-widest text-foreground">Acesso <span className="text-primary italic">VIP</span></h1>
            <p className="text-muted-foreground text-sm italic">Gestão Editorial Laeyne Studio</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-6 bg-white/5 backdrop-blur-xl p-10 rounded-[3rem] border border-primary/10 shadow-2xl">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input required type="text" className="flex h-14 w-full pl-12 bg-white/10 border-none rounded-2xl text-base text-foreground focus:ring-2 focus:ring-primary" value={username} onChange={(e) => setUsername(e.target.value)} />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="password" required className="flex h-14 w-full pl-12 bg-white/10 border-none rounded-2xl text-base text-foreground focus:ring-2 focus:ring-primary" value={password} onChange={(e) => setPassword(e.target.value)} />
                </div>
              </div>
            </div>
            <Button type="submit" className="w-full h-16 rounded-full bg-primary text-white font-black uppercase tracking-[0.2em] hover:bg-primary/90 transition-all">Entrar no Painel</Button>
          </form>
        </div>
      </main>
    );
    ...
    <SidebarProvider>
      <AdminProvider>
        <div className="flex min-h-screen w-full bg-background overflow-hidden">
          <Sidebar className="bg-sidebar border-r border-sidebar-border shadow-2xl z-50">
          ...
          <SidebarInset className="flex flex-col bg-background h-screen">
            <header className="h-20 flex items-center px-10 border-b border-primary/5 bg-background/60 backdrop-blur-xl sticky top-0 z-40">
